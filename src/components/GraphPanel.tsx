...
const GraphPanel: React.FC = () => {
    ...
    const [analysisPoints, setAnalysisPoints] = useState<{ roots: Point[], extrema: Point[] }>({ roots: [], extrema: [] });
    ...
    const handleZoomIn = () => { stateRef.current.scale *= 1.2; };
    const handleZoomOut = () => { stateRef.current.scale /= 1.2; };
    const handleReset = () => { stateRef.current = { offsetX: 0, offsetY: 0, scale: 40 }; };
    ...
    return (
        <div className="flex flex-col h-full w-full relative">
            ...
            <div className="absolute bottom-4 right-4 z-10 flex flex-col gap-2">
                <button onClick={handleZoomIn} className={`p-2 rounded-full ${theme.buttonBg} ${theme.text} shadow-lg hover:scale-110 transition-transform`}><Plus size={20} /></button>
                <button onClick={handleZoomOut} className={`p-2 rounded-full ${theme.buttonBg} ${theme.text} shadow-lg hover:scale-110 transition-transform`}><Minus size={20} /></button>
                <button onClick={handleReset} className={`p-2 rounded-full ${theme.buttonBg} ${theme.text} shadow-lg hover:scale-110 transition-transform`}><RotateCcw size={20} /></button>
            </div>
            ...
            <div
                ...
                onMouseLeave={() => { handleMouseUp(); isHovering.current = false; }}
