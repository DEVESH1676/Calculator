...
const UnitConverterMode: React.FC = () => {
    ...
    return (
        <div className="flex flex-col h-full w-full p-4 overflow-y-auto custom-scrollbar items-center">
                ...
                {categories.map((cat) => {
                    ...
                    return (
                        <button
                            ...
                            className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium transition-all whitespace-nowrap ${isActive ? `${theme.accent} text-white shadow-md` : `${theme.text} opacity-60 hover:opacity-100`
                                }`}
                        >
                            <Icon size={16} />
                            <span>{cat.id}</span>
            ...
            <div className={`w-full max-w-md p-8 rounded-3xl ${theme.bg} bg-opacity-30 border ${theme.border} relative transition-all duration-300`}>
                {/* Input Group */}
                <div className="space-y-6">
                    <div className="relative">
                        <label className={`block text-xs font-medium mb-1 ${theme.text} opacity-70`}>From</label>
                        <div className="flex gap-2">
                            ...
                            >
                                {units[activeCategory]?.map((u: string) => <option key={u} value={u} className="text-black">{u}</option>)}
                            </select>
                        </div>
                    </div>
                    ...
                    <div className="relative">
                        ...
                        <div className="flex gap-2">
                            <div className={`flex-1 p-3 rounded-xl text-lg font-mono flex items-center ${theme.glass} ${theme.primary} font-bold border ${theme.border}`}>
                ...
                <div className="mt-8 text-center opacity-50 text-xs flex flex-col items-center gap-1">
                    {activeCategory === 'Currency' && (
                        <>
                            ...
                            <button onClick={fetchRates} className="underline hover:text-white mt-1 flex items-center gap-1">
