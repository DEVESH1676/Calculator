...
const FinancialMode: React.FC = () => {
    ...
    switch (activeTool) {
        ...
        case 'sip': result = calculateSIP(); break;
        case 'cagr': result = calculateCAGR(); break;
    }
    ...
    return (
        <div className={`flex flex-col h-full w-full p-4 overflow-y-auto custom-scrollbar`}>
            ...
            <div className="flex bg-black/5 p-1 rounded-xl mb-6 self-center">
                {[
                    ...
                    return (
                        <button
                            key={tool.id}
                            onClick={() => { setActiveTool(tool.id as any); setShowDetails(false); }}
                            className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium transition-all ${isActive ? `${theme.accent} text-white shadow-md` : `${theme.text} opacity-60 hover:opacity-100`
                                }`}
                        >
                            <Icon size={16} />
            ...
            <div className="flex flex-col md:flex-row gap-6 h-full items-start justify-center">
                {/* Input Section */}
                <div className={`w-full md:w-1/2 p-6 rounded-2xl ${theme.bg} bg-opacity-30 border ${theme.border} transition-all`}>
                    {activeTool === 'emi' && (
                        <>
                            <InputField theme={theme} label="Principal Amount (P)" value={emiInputs.principal} onChange={(e: any) => setEmiInputs({ ...emiInputs, principal: e.target.value })} placeholder="e.g. 100000" />
                            <InputField theme={theme} label="Interest Rate (R %)" value={emiInputs.rate} onChange={(e: any) => setEmiInputs({ ...emiInputs, rate: e.target.value })} placeholder="e.g. 8.5" />
                            <InputField theme={theme} label="Tenure (Years)" value={emiInputs.tenure} onChange={(e: any) => setEmiInputs({ ...emiInputs, tenure: e.target.value })} placeholder="e.g. 5" />
                        </>
                    )}
                    {activeTool === 'sip' && (
                        <>
                            <InputField theme={theme} label="Monthly Investment" value={sipInputs.investment} onChange={(e: any) => setSipInputs({ ...sipInputs, investment: e.target.value })} placeholder="e.g. 5000" />
                            <InputField theme={theme} label="Expected Return Rate (R %)" value={sipInputs.rate} onChange={(e: any) => setSipInputs({ ...sipInputs, rate: e.target.value })} placeholder="e.g. 12" />
                            <InputField theme={theme} label="Time Period (Years)" value={sipInputs.years} onChange={(e: any) => setSipInputs({ ...sipInputs, years: e.target.value })} placeholder="e.g. 10" />
                        </>
                    )}
                    {activeTool === 'cagr' && (
                        <>
                            <InputField theme={theme} label="Beginning Value" value={cagrInputs.beginningValue} onChange={(e: any) => setCagrInputs({ ...cagrInputs, beginningValue: e.target.value })} placeholder="e.g. 1000" />
                            <InputField theme={theme} label="Ending Value" value={cagrInputs.endingValue} onChange={(e: any) => setCagrInputs({ ...cagrInputs, endingValue: e.target.value })} placeholder="e.g. 2000" />
                            <InputField theme={theme} label="Time Period (Years)" value={cagrInputs.years} onChange={(e: any) => setCagrInputs({ ...cagrInputs, years: e.target.value })} placeholder="e.g. 5" />
                        </>
                    )}
                </div>

                {/* Result Section */}
                <div className={`w-full md:w-1/2 p-6 rounded-2xl ${theme.glass} flex flex-col justify-start items-center text-center min-h-[400px] transition-all`}>
                    {!result ? (
                            ...
                            {activeTool === 'emi' && (
                                <>
                                    <div className="p-4 rounded-xl bg-black/10">
                                        ...
                                        <p className={`text-3xl font-bold ${theme.primary}`}>{result.emi?.toFixed(2)}</p>
                                    </div>
                                    <div className="flex justify-between text-sm p-2 border-b border-white/10">
                                        <span className={theme.text}>Total Interest</span>
                                        <span className={`font-mono ${theme.text}`}>{result.totalInterest?.toFixed(2)}</span>
                                    </div>
                                    <div className="flex justify-between text-sm p-2">
                                        <span className={theme.text}>Total Amount</span>
                                        <span className={`font-mono ${theme.text}`}>{result.totalAmount?.toFixed(2)}</span>
                                    </div>
                            ...
                            {activeTool === 'sip' && (
                                <>
                                    <div className="p-4 rounded-xl bg-black/10">
                                        ...
                                        <p className={`text-3xl font-bold ${theme.primary}`}>{result.totalAmount?.toFixed(2)}</p>
                                    </div>
                                    <div className="flex justify-between text-sm p-2 border-b border-white/10">
                                    ...
                                    <div className="flex justify-between text-sm p-2">
                                        ...
                                        <span className={`font-mono ${theme.text} text-green-400`}>+{result.totalInterest?.toFixed(2)}</span>
                                    </div>

                                    {result.chartData && (
                                        <FinancialChart data={result.chartData} type="sip" />
                                    )}
                                </>
                            )}
                            {activeTool === 'cagr' && (
                                <div className="space-y-6">
                                    <div className="p-8 rounded-xl bg-black/10">
                                        ...
                                        <p className={`text-4xl font-bold ${theme.primary}`}>{(result.cagr * 100)?.toFixed(2)}%</p>
                                    </div>
                                    {result.chartData && (
                                        <FinancialChart data={result.chartData} type="cagr" />
                                    )}
                                </div>
                            )}
                        </div>
