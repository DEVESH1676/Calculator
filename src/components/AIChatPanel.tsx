...
const calculateResult = (text: string): AIChatMessage => {
    ...
    const compoundInterestRegex = /(\d+(?:\.\d+)?(?:k|m)?)\s+(?:at|@)\s+(\d+(?:\.\d+)?)%\s+(?:for\s+)?(\d+)\s+years/i;
                ...
                {messages.map(message => (
                    ...
                    >
                        ...
                        {!message.isTyping && message.explanation && message.sender === 'bot' && (
                            <div className="mt-1 ml-2 text-xs text-white/50 animate-fade-in" style={{ animationDelay: '0.2s', animationFillMode: 'backwards' }}>
                                {message.explanation}
                            </div>
                        )}
                    </div>
